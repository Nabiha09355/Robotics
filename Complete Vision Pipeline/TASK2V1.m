...
BASKET_X   =  110;   % mm – measure your basket centre
BASKET_Y   =  110;   % mm
SLOT_SPACING = 40;   % mm between slot centres — tune to cube width + ~5 mm gap
BASKET_COLS  =  2;   % number of columns in the basket grid
% Heights (mm)
...
while true
    ...
    for i = 1:numel(valid_blobs)
        ...
        % 3D points near this blob centroid
        ...
        % Get PCA rotation from the actual blob points for tilt detection
        ...
        entry.rotation = R;        % real PCA, not eye(3)
        entry.label    = i;
    ...
    % Hover always straight down, grasp uses tilt-corrected angle
    q_pick_hover = my_findSolution(cx, cy, Z_HOVER, PHI, current_dh);
    if isempty(q_pick_hover)
        fprintf('  *** Cube %d: IK failed (hover). Skipping. ***\n', current_cube.label);
    ...
    if isempty(q_pick_grasp)
        fprintf('  *** Cube %d: IK failed (grasp). Skipping. ***\n', current_cube.label);
        flagPerceptionFigure(fig, current_cube.label, -1);
        flagged_count = flagged_count + 1;
        state = 'NEXT_TARGET'; continue
    end
    ...
    fprintf('  All waypoints planned OK.\n');  % <-- tilt_deg removed
    state = 'APPROACH_PICK';
        % --------------------------------------------------
        case 'APPROACH_PICK'
        % --------------------------------------------------
...
function phi = computeGraspAngle(rotation)
    cube_normal = double(rotation(:, 3));  % force double
    world_up    = [0; 0; 1];

    tilt_rad = acos(abs(dot(cube_normal, world_up)));
    MAX_TILT_RAD = deg2rad(60);
    ...
    phi = double(phi);  % guarantee output is always double
end