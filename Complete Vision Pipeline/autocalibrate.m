...
% Orange sticker HSV ranges. These work for a typical orange sticker
% under fluorescent lab lighting. Run tune_hsv_marker.m first if the
% calibration still can't find the sticker (see instructions below).
MARKER_H_LO  = 0.03;   % hue lower bound  (orange starts ~0.04)
MARKER_H_HI  = 0.12;   % hue upper bound  (orange ends   ~0.10, yellow ~0.17)
MARKER_S_MIN = 0.40;   % saturation min   (lower = more permissive)
MARKER_V_MIN = 0.30;   % value (brightness) min

% ALWAYS use colour mode for the orange sticker – depth mode is unreliable
% because the arm body is always the closest depth object to the camera.
...
while true
    ...
    switch state
        ...
        % --------------------------------------------------
            ...
            [clusters, ~, ~] = clusterObjects(objCloud, 1);
            if isempty(clusters)
            ...
            cube_pos(1) = CAL.sx * (cam_x_m * 1000) + CAL.dx;
            cube_pos(2) = CAL.sy * (cam_y_m * 1000) + CAL.dy;
...
% robot_y = sy * cam_y_mm + dy
    ...
    % ── Guard: camera centroids must move enough to be meaningful ────
    % If the two camera observations are nearly identical the depth
    % finder grabbed the same static object (arm body) both times.
    cam_motion = sqrt(delta_cam_x^2 + delta_cam_y^2);
    if cam_motion < 15   % mm – less than this = foreground didn't track the gripper
        error(['Calibration FAILED: camera centroid barely moved (%.1f mm)\n' ...
               'between P1 and P2, even though the robot moved %.0f mm.\n\n' ...
               'This means getGripperCentroid is tracking the arm body, not the\n' ...
               'gripper tip.  Fixes:\n' ...
               '  1. Put an orange sticker on the gripper tip and set\n' ...
               '     USE_DEPTH_CENTROID = false, MARKER_H_LO=0.04, MARKER_H_HI=0.12\n' ...
               '  2. Increase the calibration position spread (try Δ ≥ 80 mm)\n' ...
               '  3. Lower the camera or point it more directly at the workspace\n'], ...
               cam_motion, norm([delta_robot_x, delta_robot_y]));
    end

    % Guard against degenerate moves
    if abs(delta_robot_x) < 5 || abs(delta_robot_y) < 5
        error(['Calibration positions must differ by ≥5 mm in both X and Y.\n' ...
               'Current delta_robot: Δx=%.1f  Δy=%.1f'], ...
               delta_robot_x, delta_robot_y);
    end

    sx = sign(delta_robot_x) * sign(delta_cam_x);   % ±1
    sy = sign(delta_robot_y) * sign(delta_cam_y);   % ±1

    % ── Solve for dx, dy using BOTH points (average for robustness) ──
    % From P1:  dx_a = P1(1) - sx*C1(1),  dy_a = P1(2) - sy*C1(2)
    % From P2:  dx_b = P2(1) - sx*C2(1),  dy_b = P2(2) - sy*C2(2)
    % Average eliminates any residual single-frame noise.
    dx = 0.5 * ((P1(1) - sx*C1(1)) + (P2(1) - sx*C2(1)));
    dy = 0.5 * ((P1(2) - sy*C1(2)) + (P2(2) - sy*C2(2)));

    % ── Reprojection error check ─────────────────────────
    pred_x1 = sx * C1(1) + dx;   pred_y1 = sy * C1(2) + dy;
    pred_x2 = sx * C2(1) + dx;   pred_y2 = sy * C2(2) + dy;
...
% Find the XY centroid of the orange sticker on the gripper.
% Works in camera space (metres), returns result in mm.
%
% Uses HSV colour segmentation only (depth mode removed – unreliable
% because the arm body is always the closest foreground object).
%
% If this fails, run tune_hsv_marker.m with the arm at the same position
% to interactively find the correct H/S/V thresholds for your sticker
% under your lab lighting.
    ...
    % Primary mask with the configured thresholds
    marker_mask = (h >= h_lo) & (h <= h_hi) & (s > s_min) & (v > v_min);

    % ── Fallback: widen thresholds once if nothing found ──────────
    % Lighting in labs varies. If the tight range misses, try a broader
    % orange band before giving up.
    if ~any(marker_mask(:))
        fprintf('  [CAL] Primary HSV mask empty – trying wider orange range...\n');
        marker_mask = (h >= 0.02) & (h <= 0.16) & (s > 0.25) & (v > 0.20);
    end

    % Clean up speckle
    marker_mask = imopen(marker_mask,  strel('disk', 3));
    marker_mask = imclose(marker_mask, strel('disk', 5));
    marker_mask = bwareaopen(marker_mask, 80);   % keep blobs ≥ 80 px

    if ~any(marker_mask(:))
        % Save a debug image so the user can see what the camera sees
        imwrite(color_frame, 'calib_debug_frame.png');
        error(['getGripperCentroid: orange sticker not found in image.\n\n' ...
               'Debug image saved to calib_debug_frame.png\n\n' ...
               'To fix:\n' ...
               '  1. Run tune_hsv_marker.m (see below) to find the correct\n' ...
               '     MARKER_H_LO/HI, MARKER_S_MIN, MARKER_V_MIN for your\n' ...
               '     sticker under your lab lighting, then update this script.\n' ...
               '  2. Make sure the sticker is on the GRIPPER TIP (not the side)\n' ...
               '     so it faces the camera when the arm is at Z_GRASP height.\n' ...
               '  3. Verify MIN_DEPTH_M / MAX_DEPTH_M in acquirePointCloud includes\n' ...
               '     the gripper position (typically 0.30–0.70 m overhead camera).\n']);
    end

    % Keep only the largest blob (discard background orange pixels)
    cc    = bwconncomp(marker_mask);
    areas = cellfun(@numel, cc.PixelIdxList);
    [~, biggest] = max(areas);
    clean_mask = false(size(marker_mask));
    clean_mask(cc.PixelIdxList{biggest}) = true;

    % Report how many pixels were found (useful for threshold tuning)
    fprintf('  [CAL] Sticker blob: %d pixels (mask coverage OK)\n', sum(clean_mask(:)));

    % Back-project the 2D mask onto the 3D point cloud to get XY in metres
    markerCloud = maskPointCloud(ptCloud, clean_mask, intrinsics);
    ...
    if isempty(xyz)
        imwrite(color_frame, 'calib_debug_frame.png');
        error(['getGripperCentroid: colour mask found pixels but no valid depth\n' ...
               'points behind them. This usually means the depth frame has holes\n' ...
               'in that region.\n\n' ...
               'Debug image saved to calib_debug_frame.png\n\n' ...
               'To fix: lower MIN_DEPTH_M in acquirePointCloud (try 0.20 m)\n' ...
               'or raise MAX_DEPTH_M if the camera is mounted further away.\n']);
    end
...
function solutions = findJointAngles(x,y,z,phi)
    ...
    solutions=[];
    for cfg=1:2
        if cfg==1, th1=atan2(y,x);  r= sqrt(x^2+y^2);
        else,       th1=atan2(-y,-x); r=-sqrt(x^2+y^2); end
        s=z-d1; rb=r-a4*cos(phi); sb=s-a4*sin(phi);
        Dc=(rb^2+sb^2-a2^2-a3^2)/(2*a2*a3);
        if abs(Dc)>1, continue; end
        for sgn=[1,-1]
            th3=atan2(sgn*sqrt(1-Dc^2),Dc);
            th2=atan2(sb,rb)-atan2(a3*sin(th3),a2+a3*cos(th3));
            th4=phi-th2-th3;
            solutions=[solutions; mod([th1 th2 th3 th4]+pi,2*pi)-pi]; %#ok<AGROW>
        end
    end
end

function opt = my_findSolution(x,y,z,phi,curQ)
    all_s = findJointAngles(x,y,z,phi);
    if isempty(all_s), opt=[]; return; end
    lim=150*pi/180; valid=[];
    for i=1:size(all_s,1)
        c=all_s(i,:);
        if any(c<-lim)||any(c>lim), continue; end
        if checkSelfCollision(curQ,c), continue; end
        valid=[valid;c]; %#ok<AGROW>
    end
    if isempty(valid), opt=[]; return; end
    w=[4 3 2 1]; best=inf; bi=1;
    for i=1:size(valid,1)
        diff=mod((valid(i,:)-curQ)+pi,2*pi)-pi;
        cost=sum(w.*abs(diff));
        if cost<best, best=cost; bi=i; end
    end
    opt=valid(bi,:);
end

function isc=checkSelfCollision(qa,qb)
    robot=buildPincher(); robot=addPincherCollisions(robot); isc=false;
    for i=1:50
        q=qa+(i/50)*(qb-qa);
        st=checkCollision(robot,q,'SkippedSelfCollisions','adjacent');
        if any(st), isc=true; return; end
    end
end

function robot=buildPincher()
    robot=rigidBodyTree('DataFormat','row');
    a=[0 105 106 75]; alpha=[pi/2 0 0 0]; d=[145 0 0 0]; par='base';
    for i=1:4
        b=rigidBody(sprintf('link%d',i));
        j=rigidBodyJoint(sprintf('joint%d',i),'revolute');
        setFixedTransform(j,[a(i),alpha(i),d(i),0],'dh');
        b.Joint=j; addBody(robot,b,par); par=sprintf('link%d',i);
    end
end

function robot=addPincherCollisions(robot)
    d1=145;a2=105;a3=106;a4=75;r=18;
    addCollision(robot.Bodies{1},collisionCylinder(r,d1),...
        trvec2tform([0,-d1/2,0])*axang2tform([1,0,0,pi/2]));
    addCollision(robot.Bodies{2},collisionCylinder(r,a2),...
        trvec2tform([-a2/2,0,0])*axang2tform([0,1,0,pi/2]));
    addCollision(robot.Bodies{3},collisionCylinder(r,a3),...
        trvec2tform([-a3/2,0,0])*axang2tform([0,1,0,pi/2]));
    addCollision(robot.Bodies{4},collisionCylinder(r,a4),...
        trvec2tform([-a4/2,0,0])*axang2tform([0,1,0,pi/2]));
end

function dh=servo2dh(s)
    off=[0 pi/2 0 0]; dir=[1 -1 -1 -1];
    dh=mod((dir.*s)+off+pi,2*pi)-pi;
end

function s=dh2servo(dh)
    off=[0 pi/2 0 0]; dir=[1 -1 -1 -1];
    s=mod(dir.*(dh-off)+pi,2*pi)-pi;
end