function optimal_solution = my_findSolution(x, y, z, phi, currentConfig)
    ...
    all_solutions = my_findJointAngles(x, y, z, phi);
    ...
    % Define strict motor physical limits (Mapped directly from setPosition.m)
...
% Define strict motor physical limits (Mapped directly from setPosition.m)
    ...
    for i = 1:num_sols
        ...
        % Check if ANY of the 4 joints violate their specific min or max limit
        if any(candidate < limit_min) || any(candidate > limit_max)
        ...
        if my_checkSelfCollision(currentConfig, candidate)
