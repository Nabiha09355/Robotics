...
while true
    ...
    switch state
        ...
        % Back-project each 2D centroid to 3D using the point cloud
        ...
        for i = 1:numel(valid)
            k = valid(i);
            u = round(props(k).Centroid(1));   % pixel col
            v = round(props(k).Centroid(2));   % pixel row
    
            % Find 3D points within a 10px radius of the 2D centroid
            near = abs(col_all - u) < 10 & abs(row_all - v) < 10;
    
            if sum(near) < 3
                fprintf('  Cube %d: not enough depth at centroid, skipping.\n', i);
                continue
            end
    
            pos_m    = mean(xyz_all(near, :), 1);
            pos_mm   = double(pos_m * 1000);
            pos_mm(3) = 25.0;
    
            entry.position = pos_mm;
            entry.rotation = eye(3);
            entry.label    = i;
        ...
        case 'PLAN'
            ...
            % NO tilt check — go straight to IK
            ...
            if isempty(q_pick_hover)
                fprintf('  *** Cube %d unreachable (IK failed). Skipping. ***\n', current_cube.label);
