...
Z_DROP   =  60;   % release height inside basket (low = no bounce)
...
while true
    ...
    switch state
        ...
        %% ------------------------------------------------
            ...
            % Full perception pipeline (identical to M2)
            ...
            % Cluster into individual cubes (auto-K)
            [clusters, ~, K] = clusterObjects(objCloud);
            fprintf('  Found %d target cluster(s).\n', K);

            if K == 0
                state = 'DONE'; continue
            end

            % Estimate pose (centroid + PCA)
            poses = estimatePose(clusters);

            % Convert poses to robot-frame entries and build the pick queue
            raw_cubes = {};
            for k = 1:length(poses)
                if any(isnan(poses{k}.position)), continue; end
                
                % Camera frame (metres) → robot frame (mm)
                cam_x_mm = double(poses{k}.position(1) * 1000);
                cam_y_mm = double(poses{k}.position(2) * 1000);
                cam_z_mm = double(poses{k}.position(3) * 1000);  % This is DEPTH from the lens!
                
                rx = double(-cam_y_mm + CAM_DX); 
                ry = double(-cam_x_mm + CAM_DY);
                
                % --- THE DEPTH INVERSION FIX ---
                % We need to convert distance-from-lens to height-from-table.
                % TUNE THIS NUMBER: Measure the distance from your camera lens to the flat table surface!
                TABLE_DEPTH_MM = 550; % <-- Adjust this until it grabs perfectly!
