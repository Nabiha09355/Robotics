...
JAW_OPEN   =  34;      % fully open  (max ~34.59 mm)
    ...
    switch state
        ...
        % --------------------------------------------------
           ...
           % Camera frame (metres) → robot frame (mm)
            ...
            % --- THE EXACT QUADRANT FIX (CORRECTED) ---
            % Based on auto-calibration: sx = 1, sy = -1
            
            % 1. Robot X (Forward/Backward) tracks Positive Camera Y
            cx = double((cam_y_m * 1000) - 174);  
            
            % 2. Robot Y (Left/Right) tracks Negative Camera X
            cy = double(-(cam_x_m * 1000) - 84);   
