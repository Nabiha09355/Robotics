...
function success = positionJaw(position)
    ...
    % 2. Calculate the motor angle (theta1)
    cos_theta1 = (L1^2 + position^2 - L2^2) / (2 * L1 * position);
    cos_theta1 = max(min(cos_theta1, 1), -1); 
    theta1 = acos(cos_theta1);
    ...
    % 4. Safely package and send the command (TRY/CATCH REMOVED FOR DEBUGGING)
    % Read the CURRENT state of the first 4 joints
    current_arm = [arb.getpos(1), arb.getpos(2), arb.getpos(3), arb.getpos(4)];
    
    % Create the full 1x5 array 
    full_command = [current_arm, gripper_servo_angle];
    
    % Send the full 1x5 array to the robot with a 1x5 speed array!
    arb.setpos(full_command, [50, 50, 50, 50, 50]);
