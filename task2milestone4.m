...
Z_PLACE    =   80;   % drop height over basket (safe drop, not too high)
...
while true
    ...
    switch state
        ...
        % --------------------------------------------------
            ...
            if objCloud.Count == 0
                fprintf('  No target-colour objects detected in scene.\n');
                state = 'DONE';
                continue
            end

            % Auto-K clustering to find all individual cubes
            [clusters, ~, K] = clusterObjects(objCloud);
            fprintf('  Found %d cluster(s) of target colour.\n', K);
            ...
            for k = 1:length(poses)
                if any(isnan(poses{k}.position))
                    continue
                end
                entry.position = double(poses{k}.position * 1000); % → mm
                entry.rotation = poses{k}.rotation;
                entry.label    = k;
                % Fix Z: cube is resting on table, so set to known cube half-height
                entry.position(3) = 25.0;  % mm – adjust to your cube's half-height
                cube_queue{end+1} = entry; %#ok<AGROW>
            end

            fprintf('  %d cube(s) queued for pickup.\n', numel(cube_queue));

            % Show live perception figure
        ...
        % --------------------------------------------------
            ...
            cx_raw = -current_cube.position(1);
            cy_raw = -current_cube.position(2);
            cx = cx_raw - 88;
            cy = cy_raw + 53;
            ...
            % --- IK for place ---
            q_place_hover = findSolution(BASKET_X, BASKET_Y, Z_HOVER, PHI, q_pick_hover);
            ...
            q_place_grasp = findSolution(BASKET_X, BASKET_Y, Z_PLACE, PHI, q_place_hover);
        ...
        case 'APPROACH_PICK'
        % --------------------------------------------------
            fprintf('  Spinning base to face cube %d...\n', current_cube.label);

            % Step 1: spin base only (arm stays upright)
            q_spin = [q_pick_hover(1), 0, 0, 0];
            setPosition(q_spin, MOVE_SPEED);
            pause(1.5);

            % Step 2: lower to hover height above cube
...
% Move the 4 arm servos to the given DH joint angles.
% Keeps gripper (servo 5) at its current position.
    ...
    for i = 1:4
        if jointAngles(i) < limit_min(i) || jointAngles(i) > limit_max(i)
            fprintf('  Joint %d angle %.2f rad exceeds limits.\n', i, jointAngles(i));
            success = 0;
            return;
        end
    end
    ...
    try
        ...
        final_cmd = [servo_ticks(1), servo_ticks(2), ...
                     servo_ticks(3), servo_ticks(4), current_gripper];
        arb.setpos(final_cmd, [speed, speed, speed, speed, speed]);
        success = 1;
    catch err
...
% Control the parallel gripper jaw opening in mm.
    ...
    if position < min_pos || position > max_pos
        fprintf('  Jaw position %.2f mm out of range [%.2f, %.2f].\n', ...
    ...
    gripper_servo_angle = theta1;  % direction=1, offset=0
