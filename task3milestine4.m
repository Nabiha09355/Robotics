...
% 1. Tighten Hue to ignore Green and Purple
C2_H_LO  = 0.55;  C2_H_HI  = 0.68;  

% 2. INCREASE Saturation and Value to kill table shadows!
C2_S_MIN = 0.5;  C2_V_MIN = 0.25;

% --- Bin locations (mm, robot base frame, bin CENTRE) ---
BIN1_X   =  150;   BIN1_Y   =  80;    % Color-1 bin
BIN2_X   = -150;   BIN2_Y   =  80;    % Color-2 bin

% Bin grid: boxes placed in a 2-col × N-row grid inside each bin
% Slot spacing (mm) – set to cube width + 5 mm clearance
SLOT_DX  =   45;   % spacing between columns
SLOT_DY  =   45;   % spacing between rows
BIN_COLS =    2;   % number of columns in each bin grid

% --- Heights (mm) ---
Z_HOVER  =  130;   % safe transit height
Z_GRASP  =   48;   % gripper closes at this height (tune to box half-height)
Z_PLACE  =   48;   % gripper opens at this height inside bin (rest on surface)
...
% --- Camera-to-robot offset calibration (from Task 8.1 tuning) ---
CAM_OFFSET_X = -88;   % mm  – adjust as measured on your setup
CAM_OFFSET_Y =  53;   % mm
...
while true
    ...
    switch state
        ...
        %% ------------------------------------------------
            % --- Camera → Robot frame ---
            cx = -current_box.position(1) + CAM_OFFSET_X;
            cy = -current_box.position(2) + CAM_OFFSET_Y;
            cz =  current_box.position(3);   % already in mm, half-height corrected

            % For a rectangular box the base joint (θ1) must be offset by
            % grasp_yaw so the jaw aligns with the box's short axis.
            % We achieve this by adjusting the target XY so that when IK
            % solves for θ1, it naturally arrives at the correct angle.
            % Simple approach: pass grasp_yaw as a wrist-yaw offset that
            % gets baked into the IK call (θ1 = atan2(cy,cx) + grasp_yaw).
            % We rotate the target point in the XY plane by -grasp_yaw so
            % after IK the base points at the corrected angle:
            r_xy  = sqrt(cx^2 + cy^2);
            ang0  = atan2(cy, cx);
            ang_c = ang0 + current_box.grasp_yaw;  % corrected base angle
            cx_ik = r_xy * cos(ang_c);
            cy_ik = r_xy * sin(ang_c);

            % --- Tilt check (same as M2) ---
            cube_up  = current_box.rotation(:, 3);
            tilt_deg = acosd(abs(dot(cube_up, [0;0;1])));

            if tilt_deg > 35
                fprintf('  *** UNGRASPABLE: tilt=%.1f deg. Skipping. ***\n', tilt_deg);
                %flagBoxOnFigure(fig, current_box.label, sprintf('Tilt %.0f deg', tilt_deg));
            ...
            q_pick_hover = findSolution(cx_ik, cy_ik, Z_HOVER, PHI, current_dh);
            if isempty(q_pick_hover)
                fprintf('  *** UNGRASPABLE: no pick-hover IK. Skipping. ***\n');
                %flagBoxOnFigure(fig, current_box.label, 'IK fail');
                skipped_count = skipped_count + 1;
                state = 'NEXT_TARGET'; continue
            end

            q_pick_grasp = findSolution(cx_ik, cy_ik, Z_GRASP, PHI, q_pick_hover);
            ...
            q_place_hover = findSolution(slot_x, slot_y, Z_HOVER, PHI, q_pick_hover);
            if isempty(q_place_hover)
                fprintf('  ERROR: No place-hover IK. Aborting.\n');
                state = 'ERROR'; continue
            end

            q_place_grasp = findSolution(slot_x, slot_y, Z_PLACE, PHI, q_place_hover);
            if isempty(q_place_grasp)
                fprintf('  ERROR: No place-grasp IK. Aborting.\n');
                state = 'ERROR'; continue
            end

            fprintf('  Plan OK  tilt=%.1f  slot=[%.0f,%.0f]  rect=%d\n', ...
                tilt_deg, slot_x, slot_y, current_box.is_rect);
...
%%  MOTION HELPERS
%% =========================================================
function success = setPosition(jointAngles, speed)
    global arb;
    if nargin < 2, speed = 50; end

    lim_min = [-pi, -1.9, -1.8, -1.8];
    lim_max = [ pi,  1.9,  1.8,  1.8];
    for i = 1:4
        if jointAngles(i) < lim_min(i) || jointAngles(i) > lim_max(i)
            fprintf('  Joint %d = %.2f rad exceeds limits.\n', i, jointAngles(i));
            success = 0; return;
        end
    end
    try
        ticks = dh2servo(jointAngles);
        grip  = arb.getpos(5);
        arb.setpos([ticks(1:4), grip], [speed speed speed speed speed]);
        success = 1;
    catch e
...
function success = positionJaw(position)
    ...
    if position < lo || position > hi
        fprintf('  Jaw %.2f mm out of range.\n', position);
        success = 0; return;
    end
    ct = (L1^2 + position^2 - L2^2) / (2*L1*position);
    ct = max(min(ct,1),-1);
    angle = acos(ct);
    try
        arm = [arb.getpos(1), arb.getpos(2), arb.getpos(3), arb.getpos(4)];
        arb.setpos([arm, angle], 50);
        success = 1;
    catch
        disp('  Gripper comm error.');
        success = 0;
    end
end
